...
class OAuth1ConsumerBlueprint(BaseOAuthConsumerBlueprint):
    ...
    def authorized(self):
        ...
        """
        import flask
        oauth_token, oauth_verifier = flask.request.args.get('oauth_token'), flask.request.args.get('oauth_verifier')
        if self.redirect_url:
            next_url = self.redirect_url
        elif self.redirect_to:
            next_url = url_for(self.redirect_to)
        ...
        try:
            token = self.session.fetch_access_token(
                self.access_token_url + f"?oauth_token={oauth_token}&oauth_verifier={oauth_verifier}", should_load_token=False
            )
